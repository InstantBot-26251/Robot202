 ...
 * ATLivestreamTeleOp
 *
 * - Uses ATLivestream (VisionProcessor + CameraStreamSource) to supply frames to Dashboard
 * - Adds AprilTagProcessor to VisionPortal for detections
 * - Shows telemetry (driver + dashboard) when tags are seen
 * - gamepad1.a locks current detection; gamepad1.b clears lock
 *
 * Requirements:
 * - ATLivestream class present in org.firstinspires.ftc.teamcode.vision
 * - A webcam configured in the robot config named "Webcam 1" (change WEBCAM_NAME if needed)
 * - FtcDashboard reachable on the network
 */
...
public class ATTestingOpMode extends OpMode {
    ...
    @Override
    public void loop() {
        // Edge detection for lock/clear buttons
        boolean a = gamepad1.a;
        boolean b = gamepad1.b;

        if (a && !prevA) {
            // lock current best detection if any
            if (aprilTagProcessor != null) {
                List<AprilTagDetection> detections = aprilTagProcessor.getDetections();
                if (detections != null && !detections.isEmpty()) {
                    // pick best (first) detection to lock — you can change selection strategy
        ...
        if (aprilTagProcessor == null) {
            ...
            if (detections == null || detections.isEmpty()) {
                ...
                // Show up to first 3 detections for readability
                int shown = 0;
